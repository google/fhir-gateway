...
public class BearerAuthorizationInterceptor {
   ...
   BearerAuthorizationInterceptor(String gcpFhirStore, String tokenIssuer) throws IOException {
      ...
      jwtVerifier = JWT.require(Algorithm.RSA256(issuerPublicKey, null)).withIssuer(tokenIssuer).build();
   }
   ...
   private RSAPublicKey fetchAndDecodePublicKey() throws IOException {
      ...
      try {
         ...
         if (keyStr == null) {
            ExceptionUtil.throwRuntimeExceptionAndLog(logger, "Cannot find 'public_key' in issuer metadata.");
         }
         KeyFactory keyFactory = KeyFactory.getInstance(keyAlgorithm);
         EncodedKeySpec keySpec = new X509EncodedKeySpec(Base64.getDecoder().decode(keyStr));
         return (RSAPublicKey) keyFactory.generatePublic(keySpec);
      } catch (URISyntaxException e) {
         ExceptionUtil.throwRuntimeExceptionAndLog(logger, "Error in token issuer URI " + tokenIssuer, e);
